...
export default function SetAdminPassword() {
  ...
  async function handleSubmit(e) {
    ...
    try {
      ...
      await axios.post(`${API}/api/admin/set-password`, 
      ...
      <div className="bg-white rounded-2xl shadow-xl border border-gray-200 p-10 w-full max-w-md">
        ...
        <div className="bg-yellow-50 border-2 border-yellow-400 rounded-xl p-4 mb-6">
          ...
          <p className="text-yellow-700 text-xs mt-1">
            You are using the default admin password. Please set a new secure password before continuing.
          </p>
        </div>

        <h1 className="text-xl font-bold text-gray-900 text-center mb-1">Set New Admin Password</h1>
        <p className="text-gray-400 text-sm text-center mb-6">This password will be used for all future admin logins.</p>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <div className="bg-gray-50 rounded-lg p-3 text-xs text-gray-500 space-y-1">
            <p className={form.password.length >= 8 ? 'text-green-600 font-semibold' : ''}>
              {form.password.length >= 8 ? '✅' : '○'} At least 8 characters
            </p>
            <p className={/[A-Z]/.test(form.password) ? 'text-green-600 font-semibold' : ''}>
              {/[A-Z]/.test(form.password) ? '✅' : '○'} At least one uppercase letter
            </p>
            <p className={/[0-9]/.test(form.password) ? 'text-green-600 font-semibold' : ''}>
